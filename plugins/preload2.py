...
@LastEditTime : 2020-01-13 17:35:28
'''
import asyncio
...
from general import requests_headers
...
def run(url, cms_list, arg_dict, sums, return_dict=None):
    global THREAD, SILENCE, SUM, SUCCESS, SESSION, STOP, TEMP
    # --------------------------------------------
    SUCCESS = {}
    SUM = sums
    ...
    try:
        ...
        while (SUM[0] < SUM[1] and len(enumerate()) > 1):
            sleep(0.2)
            count += 1
            if (count >= 500):  # 超时100秒
